...
export function TopicDirectory({ title, intro, links }: { title: string; intro: string; links: Array<{ label: string; href: string; icon: typeof FileText }> }) {
  return <section className="container py-12 md:py-16"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">Information directory</p><h1 className="mt-3 font-display text-4xl font-bold leading-tight text-secondary md:text-5xl">{title}</h1><p className="mt-4 text-base leading-7 text-slate-600">{intro}</p></div><div className="mt-8 grid gap-4 md:grid-cols-3">{links.map(({ label, href, icon: Icon }) => <Link key={href} to={href} className="group rounded-xl border border-border bg-card p-5 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-primary/60 hover:shadow-[var(--shadow-elevated)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"><div className="flex items-center justify-between"><Icon className="h-5 w-5 text-primary" /><LinkIcon className="h-4 w-4 text-slate-400 transition-transform duration-200 group-hover:translate-x-1 group-hover:text-secondary" /></div><h2 className="mt-5 font-display text-xl font-bold text-secondary">{label}</h2><p className="mt-2 text-sm leading-6 text-slate-600">Read the official overview and practical guidance.</p><span className="mt-5 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-secondary group-hover:text-primary">Open route <LinkIcon className="h-3.5 w-3.5" /></span></Link>)}</div></section>;
}
